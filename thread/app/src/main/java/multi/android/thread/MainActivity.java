...
public class MainActivity extends AppCompatActivity {
    ...
    //작업 쓰레드가 핸들러에게 View에 대한 변경을 요청한다.
    //핸들러는 작업 쓰레드로부터 받은 요청 정보를 꺼내서 뷰를 변경한다.
    ...
    public void userHandler(View view){
        new Thread(new Runnable() {
            ...
            public void run() {
                for(progressVal=1;progressVal<=100;progressVal++){
                    //handler가 갖고있는 Message객체를 매개변수로 전달하며 작업을 의뢰
            ...
            public void run() {
                for(int i=1;i<=100;i++){
                   //변경할 뷰의 정보나 Handler에게 전달한 데이터를
                    ...
                    msg.what =1; //what : handler에게 작업을 의뢰한 쓰레드를 구분하기 위한 코드
                    msg.arg1 = i; //전달할 데이터
